...
func set_base_state(dangerous_animals : Array[Animal]):
	if curr_energy_level <= 0 or curr_health <= 0:
	...
	elif curr_hunger/max_hunger < seek_nutrition_threshold and curr_hunger < curr_hydration:
		animal_state = Animal_Base_States.HUNGRY
	elif curr_hydration/max_hydration < seek_hydration_threshold and curr_hydration <= curr_hunger:
...
func process_cadaver(delta : float):
	...
	corpse_timer += delta
	if corpse_timer >= corpse_max_timer:
...
func get_tiles_from_senses() -> Array[World.Tile_Properties]:
	...
	var tiles_in_direction : int = sense_range / World.tile_size.x
